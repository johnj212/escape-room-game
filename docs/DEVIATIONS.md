...
### D-6 — Puzzle-2 arm window: 1.5 s → 3.0 s (user playtest tuning)
- **Spec:** §3.14 / §6 Phase-2 gate: "three role-keyed scanners armed within a **1.5 s** rolling window."
...
- **Why:** Real-human playtest tuning. 1.5 s demands near-frame-perfect coordination across three networked players (or six keystrokes in solo swap); the playtest evidence is the user widening it directly. The interdependence *shape* is unchanged — the exhaustive 2-role-subset insufficiency proof and the authority probe are window-agnostic and still pass.
- **Cost / to revisit:** The puzzle is easier than the brief's number; tension relies more on the latch/lockout rhythm. Pinned by a literal-value regression test (`scannerPuzzle.test.js` asserts `ARM_WINDOW_MS === 3000` referencing this entry) so the constant can never drift silently again — found by the Phase-2 gate-verifier as an undocumented spec change, which this entry retroactively papers. If the user wants 1.5 s back (e.g. as a hard-mode), revert `9b04c77`'s one-liner and update the pin test + this entry.
