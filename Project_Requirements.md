...
14. **Puzzle 2 — Tri-Vector Hand Scanners** (simultaneous, **3-role**: all 3 scanners must be armed within a 3.0 s rolling window (amended 2026-07-09 from 1.5 s — user playtest, mobile input timing; see docs/DEVIATIONS.md D-6); failure → lockout cooldown. Each scanner *latches* armed for a few seconds after activation, so a solo-swap player arms all three across character swaps and the same identical puzzle resolves — never a relaxed or sequential solo variant)
...
| 2 | high | Puzzle 2 (scanners) + 1 → 2 chain wired server-side | e2e solves P1 → P2; simultaneous 3.0 s window (amended 2026-07-09, D-6) + latched-arm solo-swap solve + lockout verified; a test proves 2 roles cannot complete it (Pillar A, 3-role); server-authoritative; reference-delta |
